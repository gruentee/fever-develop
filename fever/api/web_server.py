...
def fever_web_api(predict_function):
    ...
    def predict():
        ...
        if all(map(lambda instance: isinstance(instance, str), request.json["instances"])):
            instances.extend([FEVERRequestInstance(**{"id": idx, "claim": instance}) for idx, instance in
                              enumerate(request.json["instances"])])
            ...
            return json_response(wrap_error(
                "Instances field must contain a list of strings or objects containing 'id':int and 'claim':str fields"),
                                 400)
    ...
    def predict_text():
        ...
        predictions = predict_function(get_from_text=True, text=text)
